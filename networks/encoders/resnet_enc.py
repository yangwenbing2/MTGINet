...
class ResNet_D(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, late_downsample=False):
        ...
        self.conv1 = SpectralNorm(nn.Conv2d(3, 32, kernel_size=3,
    ...
    def forward(self, x):
        ...
        x = self.bn3(x)
        x = self.activation(x)
        x2 = self.layer1(x)  # N x 64 x 128 x 128
...
if __name__ == "__main__":
    ...
    inputs = torch.rand(8, 3, 512, 512).to(device)

    model = ResNet_D(BasicBlock, [3, 4, 4, 2]).cuda()
    pred = model(inputs)
